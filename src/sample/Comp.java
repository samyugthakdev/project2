...
public class Comp extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try
		{
		    ...
		    String c_dead=request.getParameter("date");
		    String c_mem=request.getParameter("num");
		    String c_req=request.getParameter("req");
		    String c_add=request.getParameter("text");
		    int rs=st.executeUpdate("insert into comp_reg(c_name,c_phn,c_mail,c_deadline,c_mem,c_req,c_add)values('"+c_name+"','"+c_num+"','"+c_mailid+"','"+c_dead+"','"+c_mem+"','"+c_req+"','"+c_add+"')");
		    out.print("<h1>Insert successfull</h1>");
		    ServletContext sc = getServletContext();
	          sc.getRequestDispatcher("/comp_s.html").forward(request, response);
		    con.close();
		}
		catch(Exception e)
		{
